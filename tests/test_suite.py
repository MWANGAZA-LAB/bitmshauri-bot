...
class BaseTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_db_path = os.path.join(tempfile.gettempdir(), 'test_bitmshauri.db')
...
class TestDatabaseManager(BaseTestCase):
    ...
    def test_database_initialization(self):
        ...
        expected_tables = ['users', 'user_activities', 'lesson_progress', 'quiz_results', 'price_alerts']
    ...
    def test_lesson_progress(self):
        ...
        result = db_manager.update_lesson_progress(self.test_user_id, lesson_key, True)
        self.assertTrue(result)
        
        # Check progress
        progress = db_manager.get_user_progress(self.test_user_id)
        self.assertIsNotNone(progress)
        self.assertIn(lesson_key, [p['lesson_key'] for p in progress.get('lessons', [])])
...
class TestRateLimiter(BaseTestCase):
    ...
    def test_rate_limiting_different_actions(self):
        ...
        message_result = rate_limiter.check_rate_limit(self.test_user_id, 'message')
...
class TestBitcoinCalculator(BaseTestCase):
    ...
    def test_usd_to_btc_calculation(self, mock_price):
        ...
        self.assertAlmostEqual(result['converted_amount'], 0.002, places=6)  # 100/50000
...
class TestProgressTracker(BaseTestCase):
    ...
    def test_lesson_completion(self):
        ...
        completed_lessons = [l['lesson_key'] for l in progress.get('completed_lessons', [])]
    ...
    def test_quiz_completion(self):
        ...
        result = self.tracker.complete_quiz(self.test_user_id, quiz_name, score)
        self.assertTrue(result)
        ...
        test_quiz_result = next((q for q in quiz_results if q['quiz_name'] == quiz_name), None)
...
class TestMultiLanguageBot(BaseTestCase):
    ...
    def test_language_detection_swahili(self):
        ...
        language = multi_lang_bot.language_detector.detect_language(swahili_text)
        self.assertEqual(language, 'sw')
    
    def test_language_detection_english(self):
        """Test English language detection"""
        english_text = "Hello, I need help with Bitcoin"
        language = multi_lang_bot.language_detector.detect_language(english_text)
...
class TestCommunityFeatures(BaseTestCase):
    ...
    def setUp(self):
        ...
        # Add test user
        community_manager.db_manager.add_user(self.test_user_id, self.test_username)
    ...
    def test_group_messaging(self):
        ...
        # Get messages
        messages = community_manager.get_group_messages(group_id, self.test_user_id)
...
class TestIntegration(BaseTestCase):
    ...
    def test_complete_learning_flow(self):
        ...
        # Complete a lesson
        lesson_result = self.tracker.complete_lesson(self.test_user_id, 'intro')
        self.assertTrue(lesson_result)
        
        # Take a quiz
        quiz_result = self.tracker.complete_quiz(self.test_user_id, 'msingi', 90)
        self.assertTrue(quiz_result)
...
def run_all_tests():
    ...
    # Add async tests
    async_tests = unittest.TestLoader().loadTestsFromTestCase(TestAsyncComponents)
