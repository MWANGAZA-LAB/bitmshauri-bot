...
async def start(update: Update, context: CallbackContext):
    await update.message.reply_text(
        SWAHILI_CONTENT["welcome"], reply_markup=create_menu()
    )
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    if text == "💰 Bei ya Bitcoin":
        ...
        await update.message.reply_text(
            "Hii ni toleo rahisi la BitMshauri. Tunaendelea kuifanya bora!"
        )
    elif text == "❓ Usaidizi":
        await update.message.reply_text("Tuma /start kuanza tena.")
    else:
        await update.message.reply_text(
            SWAHILI_CONTENT["unknown_command"], reply_markup=create_menu()
        )
