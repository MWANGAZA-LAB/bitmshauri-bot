...
from app.services.multi_language import multi_lang_bot, get_user_language, set_user_language
...
# Conversation states
(WAITING_FOR_GROUP_NAME, WAITING_FOR_GROUP_DESCRIPTION, 
 WAITING_FOR_FEEDBACK, WAITING_FOR_CALCULATION) = range(4)
...
class EnhancedBitMshauriBot:
    ...
    async def start_command(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            user_language = get_user_language(user_id, update.message.text if update.message else None)
            ...
            reply_markup = ReplyKeyboardMarkup(menu_keyboard, resize_keyboard=True) if menu_keyboard else None
            ...
            # Log user start
            app_logger.log_user_action(user_id, 'bot_started', {
                'username': username,
                'language': user_language
            })
            
        except Exception as e:
            app_logger.log_error(e, {"operation": "start_command", "user_id": update.effective_user.id})
    ...
    async def handle_message(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            if message_text in self._get_all_menu_options(user_id):
                ...
                await self._handle_general_message(update, context, message_text)
                
        except Exception as e:
            app_logger.log_error(e, {"operation": "handle_message", "user_id": update.effective_user.id})
            error_msg = multi_lang_bot.get_response(update.effective_user.id, 'error_occurred')
    ...
    def _get_all_menu_options(self, user_id: int) -> List[str]:
        ...
        try:
            ...
            secondary_menu = multi_lang_bot.get_secondary_menu_keyboard(user_id) or []
    ...
    def _is_calculation_request(self, text: str) -> bool:
        ...
        calc_keywords = ['calculate', 'convert', 'hesabu', 'badili', 'usd', 'btc', 'bitcoin', 'dola']
        return any(keyword in text.lower() for keyword in calc_keywords)
    
    async def _handle_menu_option(self, update: Update, context: CallbackContext, option: str):
    ...
    async def _handle_price_request(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            if price_data:
                ...
                price_message = multi_lang_bot.format_price_message(user_id, price_data)
                
                # Create price alert button
                keyboard = [[
                    InlineKeyboardButton("🔔 Weka Kumbuka za Bei", callback_data="set_price_alert")
                ]]
                ...
                # Log price request
                app_logger.log_user_action(user_id, 'price_requested', {'price_usd': price_data.get('usd')})
            else:
            ...
            await update.message.reply_text("Tatizo la kupata bei ya Bitcoin. Jaribu tena.")
    
    async def _handle_calculation(self, update: Update, context: CallbackContext, text: str):
        ...
        try:
            ...
            if result:
                ...
                calc_message = multi_lang_bot.format_calculation_result(user_id, result)
                await update.message.reply_text(calc_message, parse_mode='Markdown')
                
                app_logger.log_user_action(user_id, 'calculation_performed', {
                    'calculation': text,
                    'result': result
                })
            else:
    ...
    async def _handle_quiz_start(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            if not quiz_questions:
                await update.message.reply_text("Hakuna maswali ya jaribio kwa sasa.")
                return
    ...
    async def _send_quiz_question(self, update: Update, context: CallbackContext, user_id: int):
        ...
        try:
            ...
            # Format question
            question_text = f"❓ **Swali {current_q + 1}/{len(questions)}**\n\n"
            ...
            keyboard = []
            for i, option in enumerate(question['options'], 1):
                keyboard.append([InlineKeyboardButton(f"{i}. {option}", callback_data=f"quiz_{i-1}")])
            
            # Add audio button
            keyboard.append([InlineKeyboardButton("🎵 Sikiza Swali", callback_data="quiz_audio")])
    ...
    async def _handle_audio_request(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            # Show voice options
            keyboard = [
                [InlineKeyboardButton("🎓 Mshauri (Kawaida)", callback_data="voice_mshauri")],
                [InlineKeyboardButton("⚡ Haraka", callback_data="voice_haraka")],
                [InlineKeyboardButton("👴 Mzee", callback_data="voice_mzee")],
                [InlineKeyboardButton("👦 Kijana", callback_data="voice_kijana")]
            ]
    ...
    async def _handle_community_features(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            keyboard = [
                [InlineKeyboardButton("👥 Orodha ya Makundi", callback_data="list_groups")],
                [InlineKeyboardButton("➕ Unda Kundi", callback_data="create_group")],
                [InlineKeyboardButton("📊 Makundi Yangu", callback_data="my_groups")],
                [InlineKeyboardButton("🏆 Changamoto", callback_data="challenges")]
            ]
            
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            message = ("🌟 **Vipengele vya Jamii**\n\n"
    ...
    async def _handle_lesson_request(self, update: Update, context: CallbackContext, lesson_text: str):
        ...
        try:
            ...
            if lesson:
                ...
                keyboard = [
                    [InlineKeyboardButton("🎵 Sikiza Somo", callback_data=f"audio_lesson_{lesson_key}")],
                    [InlineKeyboardButton("📝 Jaribio", callback_data="start_quiz")],
                    [InlineKeyboardButton("↩️ Rudi Menyu", callback_data="main_menu")]
                ]
                ...
                app_logger.log_user_action(user_id, 'lesson_viewed', {
                    'lesson_key': lesson_key,
                    'lesson_title': lesson.get('title', lesson_key)
                })
            else:
                await update.message.reply_text("Samahani, somo hilo halipatikani.")
