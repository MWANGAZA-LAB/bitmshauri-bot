...
from app.bot.content_swahili import (
    LESSONS,
    MENU_KEYBOARD,
    QUIZZES,
    DAILY_TIPS,
...
def create_secondary_menu():
    return ReplyKeyboardMarkup(
        SECONDARY_MENU_KEYBOARD, resize_keyboard=True, one_time_keyboard=False
    )
...
async def start(update: Update, context: CallbackContext):
    ...
    await update.message.reply_photo(
        photo=logo_url, caption=welcome_msg, reply_markup=create_menu()
    )
...
async def back_to_main_menu(update: Update, context: CallbackContext):
    await update.message.reply_text(
        "Unakaribishwa tena kwenye menyu kuu.", reply_markup=create_menu()
    )
...
# --- Audio Functions ---
async def send_lesson_with_audio(
    update: Update, context: CallbackContext, lesson_key: str
):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "🔊 Sikiliza Audio", callback_data=f"audio_{lesson_key}"
            )
        ],
...
async def handle_audio_callback(update: Update, context: CallbackContext):
    ...
    if query.data.startswith("audio_"):
        ...
        await query.edit_message_text(
            "✅ Umechagua kusoma tu. Endelea na mafunzo!"
        )
...
async def generate_and_send_audio(
    query, context: CallbackContext, lesson_key: str
):
...
async def show_audio_menu(update: Update, context: CallbackContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "🔊 Kwa Nini Bitcoin?", callback_data="audio_kwa_nini_bitcoin"
            )
        ],
        [
            InlineKeyboardButton(
                "🔊 Bitcoin ni nini?", callback_data="audio_bitcoin_ni_nini"
            )
        ],
        [
            InlineKeyboardButton(
                "🔊 P2P Inafanyaje", callback_data="audio_p2p_inafanyaje"
            )
        ],
        [
            InlineKeyboardButton(
                "🔊 Aina za Pochi", callback_data="audio_kufungua_pochi"
            )
        ],
        [
            InlineKeyboardButton(
                "🔊 Usalama wa Pochi", callback_data="audio_usalama_pochi"
            )
        ],
        [
            InlineKeyboardButton(
                "🔊 Blockchain", callback_data="audio_blockchain_usalama"
            )
        ],
...
async def handle_purchase_platform(
    update: Update, context: CallbackContext, platform: str
):
    platform_info = {
        # Purchase platforms
        "Bitika": {
            "url": "bitika.xyz",
            "desc": "Jukwaa la kununua na kuuza Bitcoin kwa M-Pesa",
            "type": "purchase",
        },
        "Bitsacco": {
            "url": "bitsacco.com",
            "desc": "Jukwaa la P2P la Bitcoin kwa wafanya biashara",
            "type": "purchase",
        },
        # Wallet platforms
        "Blink": {
            "url": "play.google.com/store/apps/details?id=com.galoyapp&pli=1",
            "desc": "Pochi ya Bitcoin ya haraka na rahisi",
            "type": "wallet",
        },
        "Fedi": {
            "url": "fedi.xyz",
            "desc": "Pochi ya jamii ya Bitcoin",
            "type": "wallet",
        },
        "Phoenix": {
            "url": "phoenix.acinq.co",
            "desc": "Pochi ya Lightning ya Bitcoin",
            "type": "wallet",
        },
        "Wallet of Satoshi": {
            "url": "walletofsatoshi.com",
            "desc": "Pochi rahisi ya Lightning",
            "type": "wallet",
        },
        # Spending platforms
        "Tando": {
            "url": "tando.africa",
            "desc": "Tumia Bitcoin kwa M-Pesa - kubadilisha Bitcoin kuwa pesa taslimu",
            "type": "spending",
        },
    }
...
async def ai_answer_handler(update: Update, context: CallbackContext):
    ...
    prompt = (
        f"Jibu swali lifuatalo kuhusu Bitcoin kwa Kiswahili: {user_question}"
    )
    headers = {
    ...
    data = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "Jibu maswali kuhusu Bitcoin kwa Kiswahili, kwa ufupi na urafiki.",
            },
        ...
        if "choices" in result and result["choices"]:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "🔊 Sikiliza Audio",
                        callback_data=f"ai_audio_{hash(answer)}",
                    )
                ],
                [
                    InlineKeyboardButton(
                        "📖 Soma tu", callback_data="text_only"
                    )
                ],
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            ...
            error_msg = result.get("error", {}).get(
                "message", "Hakuna jibu lililopatikana kutoka AI."
            )
            await update.message.reply_text(
                f"Samahani, AI haikuweza kujibu: {error_msg}"
            )

    except Exception as e:
        logger.error(f"AI handler error: {e}")
        await update.message.reply_text(
            "Samahani, kuna tatizo na huduma ya AI. Tafadhali jaribu tena baadaye."
        )
...
async def ask_quiz_question(
    ...
    keyboard = [
        [str(i + 1) for i in range(len(q_data["options"]))],
        ["❌ Acha Jaribio"],
    ]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    await update.message.reply_text(
        q_text, reply_markup=reply_markup, parse_mode="Markdown"
    )
...
async def handle_quiz_answer(update: Update, context: CallbackContext):
    ...
    if state["current_question"] < len(quiz):
        ...
        emoji, message = (
            ("🎉", "Hongera! Umefanya kazi nzuri sana!")
            if percentage >= 80
            else (
                ("👍", "Vizuri! Unaendelea vizuri!")
                if percentage >= 50
                else ("💪", "Endelea kujifunza, utafanikiwa!")
            )
        )
        result = (
        ...
        await update.message.reply_text(
            result, reply_markup=create_menu(), parse_mode="Markdown"
        )
        del user_quiz_state[user_id]
...
async def send_daily_tip(bot, user_id, chat_id):
    try:
        ...
        await bot.send_message(
            chat_id=chat_id, text=tip, parse_mode="Markdown"
        )
...
def safe_get_lesson(lesson_key):
    ...
    return LESSONS.get(lesson_key, {}).get(
        "content", f"Samahani, somo '{lesson_key}' halipatikani."
    )


# --- Enhanced Callback Handler ---
async def handle_audio_callback_enhanced(
    ...
    if query.data.startswith("audio_"):
        ...
        if ai_response:
            try:
                await query.edit_message_text(
                    "🎵 Inaandaa sauti ya AI... Subiri kidogo..."
                )
                ...
                await query.edit_message_text(
                    "❌ Imeshindwa kutengeneza sauti ya AI."
                )
    elif query.data == "text_only":
        await query.edit_message_text(
            "✅ Umechagua kusoma tu. Endelea na mafunzo!"
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    responses = {
        ...
        "🌍 Kwa Nini Bitcoin?": create_audio_lesson_response(
            "kwa_nini_bitcoin"
        ),
        ...
        "🔐 Teknolojia ya Blockchain": create_audio_lesson_response(
            "blockchain_usalama"
        ),
        # New audio menu
        ...
        "💡 Kidokezo cha Leo": lambda u, c: send_daily_tip(
            c.bot, u.effective_user.id, u.effective_chat.id
        ),
        ...
        "Wallet of Satoshi": lambda u, c: handle_purchase_platform(
            u, c, "Wallet of Satoshi"
        ),
        # Spending platforms
    ...
    if text == "❓ Maswali Mengine":
        if update.message:
            await update.message.reply_text(
                safe_get_lesson("maswali_mengine"), parse_mode="Markdown"
            )
...
def init_bot():
    try:
        ...
        application.add_handler(
            CallbackQueryHandler(handle_audio_callback_enhanced)
        )
