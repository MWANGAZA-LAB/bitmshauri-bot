...
from app.enhanced_database import (
    get_user_stats,
...
class UserProgressTracker:
    ...
    async def get_user_progress(self, user_id: int) -> Dict:
        ...
        try:
            ...
            logger.log_error(
                e, {"operation": "get_user_progress", "user_id": user_id}
            )
            return self._create_empty_progress()
    ...
    async def _calculate_learning_streak(self, user_id: int) -> Dict:
        ...
        try:
            ...
            with db_connection() as cursor:
                ...
                for i in range(1, len(activity_dates)):
                    prev_date = datetime.strptime(
                        activity_dates[i - 1], "%Y-%m-%d"
                    ).date()
                    curr_date = datetime.strptime(
                        activity_dates[i], "%Y-%m-%d"
                    ).date()
                ...
                return {
                    ...
                    "last_activity": (
                        activity_dates[0] if activity_dates else None
                    ),
                }
            ...
            logger.log_error(
                e,
                {
                    "operation": "_calculate_learning_streak",
                    "user_id": user_id,
                },
            )
            return {
    ...
    async def _check_achievements(
        ...
        try:
            ...
            with db_connection() as cursor:
                ...
                if calc_uses >= 10:
                    achievements.append(
                        self.achievement_definitions["calculator_user"]
                    )
            ...
            if streak_info.get("current_streak", 0) >= 3:
                achievements.append(
                    self.achievement_definitions["consistent_learner"]
                )
            ...
            logger.log_error(
                e, {"operation": "_check_achievements", "user_id": user_id}
            )
            return []
    ...
    async def _calculate_module_progress(self, user_id: int) -> Dict:
        ...
        try:
            ...
            with db_connection() as cursor:
                ...
                for module_name, lessons in self.lesson_modules.items():
                    completed_in_module = len(
                        [l for l in lessons if l in completed_lessons]
                    )
            ...
            logger.log_error(
                e,
                {
                    "operation": "_calculate_module_progress",
                    "user_id": user_id,
                },
            )
            return {}
    ...
    def _calculate_overall_progress(
        self, stats: Dict, module_progress: Dict
    ) -> float:
        """Calculate overall progress percentage"""
        try:
            ...
            total_lessons = sum(
                len(lessons) for lessons in self.lesson_modules.values()
            )
            lesson_progress = min(
                (
                    stats.get("lessons", {}).get("completed_lessons", 0)
                    / total_lessons
                )
                * 100,
                100,
            ...
            quiz_progress = min(
                stats.get("quizzes", {}).get("avg_score", 0), 100
            )
            ...
            if module_progress:
                ...
                module_completion = (
                    completed_modules / len(module_progress)
                ) * 100
    ...
    def _get_next_goals(
        self, stats: Dict, achievements: List[Dict]
    ) -> List[str]:
    ...
    def format_progress_report(
        self, progress: Dict, user_name: str = "Mwanafunzi"
    ) -> str:
