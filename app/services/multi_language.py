...
class LanguageDetector:
    ...
    def __init__(self):
        ...
        self.swahili_regex = re.compile('|'.join(self.swahili_patterns), re.IGNORECASE)
    ...
    def detect_language(self, text: str) -> str:
        ...
        try:
            ...
            if any(word in text_clean for word in ['habari', 'mambo', 'hujambo', 'niaje']):
                return 'sw'
...
class MultiLanguageBot:
    ...
    def set_user_language(self, user_id: int, language: str) -> bool:
        ...
        try:
            if language in self.supported_languages:
                ...
                logger.log_user_action(user_id, 'language_set', {'language': language})
                return True
            return False
        except Exception as e:
    ...
    def get_user_language(self, user_id: int, auto_detect_text: str = None) -> str:
        ...
        try:
            ...
            if auto_detect_text:
                detected_lang = self.language_detector.detect_language(auto_detect_text)
    ...
    def get_response(self, user_id: int, template_key: str, **kwargs) -> str:
        ...
        try:
            ...
            template = self.response_templates.get(language, {}).get(template_key, "")
            ...
            return self.response_templates['sw'][template_key] if template_key in self.response_templates['sw'] else ""
    
    def get_content(self, user_id: int, content_type: str, content_key: str = None, auto_detect_text: str = None):
        """Get localized content for user"""
        try:
            language = self.get_user_language(user_id, auto_detect_text)
            
            if content_key:
                return content_manager.get_content(content_type, content_key, language)
            else:
    ...
    def get_lesson(self, user_id: int, lesson_key: str, auto_detect_text: str = None):
        """Get localized lesson"""
        return self.get_content(user_id, 'lessons', lesson_key, auto_detect_text)
    
    def get_quiz(self, user_id: int, quiz_name: str, auto_detect_text: str = None):
        """Get localized quiz"""
        return self.get_content(user_id, 'quizzes', quiz_name, auto_detect_text)
    
    def get_menu_keyboard(self, user_id: int, auto_detect_text: str = None):
        """Get localized menu keyboard"""
        return self.get_content(user_id, 'menu_keyboard', None, auto_detect_text)
    
    def get_secondary_menu_keyboard(self, user_id: int, auto_detect_text: str = None):
        """Get localized secondary menu keyboard"""
        return self.get_content(user_id, 'secondary_menu_keyboard', None, auto_detect_text)
    ...
    def format_calculation_result(self, user_id: int, calculation_result: Dict) -> str:
    ...
    def get_language_stats(self) -> Dict:
        ...
        try:
            ...
            for language in self.supported_languages:
                count = sum(1 for lang in self.user_languages.values() if lang == language)
...
def get_localized_content(user_id: int, content_type: str, content_key: str = None, auto_detect_text: str = None):
    """Get localized content"""
    return multi_lang_bot.get_content(user_id, content_type, content_key, auto_detect_text)
