...
class CommunityManager:
    ...
    def __init__(self):
        ...
        self.roles = {
            ...
            "moderator": {
                "level": 2,
                "permissions": ["read", "post", "moderate"],
            },
            "admin": {
                "level": 3,
                "permissions": ["read", "post", "moderate", "manage"],
            },
        }
    ...
    def join_study_group(self, user_id: int, group_id: int) -> bool:
        ...
        try:
            ...
            if (
                not group_info or not group_info[2]
            ...
            logger.log_user_action(
                user_id,
                "group_joined",
                {"group_id": group_id, "group_name": group_info[0]},
            )
    ...
    def leave_study_group(self, user_id: int, group_id: int) -> bool:
        ...
        try:
            ...
            logger.log_user_action(
                user_id, "group_left", {"group_id": group_id}
            )
            return True
    ...
    def get_group_messages(
        self, group_id: int, user_id: int, limit: int = 20, offset: int = 0
    ) -> List[Dict]:
    ...
    def get_public_groups(
        self, language: str = None, limit: int = 20
    ) -> List[Dict]:
    ...
    def join_challenge(
        self, user_id: int, challenge_id: int, group_id: int = None
    ) -> bool:
    ...
    def get_active_challenges(
        self, language: str = None, limit: int = 10
    ) -> List[Dict]:
    ...
    def get_community_stats(self) -> Dict:
        ...
        try:
            ...
            # Group statistics
            cursor.execute(
                "SELECT COUNT(*) FROM study_groups WHERE is_public = 1"
            )
            public_groups = cursor.fetchone()[0]

            cursor.execute(
                "SELECT COUNT(*) FROM group_memberships WHERE is_active = 1"
            )
            total_memberships = cursor.fetchone()[0]

            cursor.execute(
                "SELECT COUNT(*) FROM group_discussions WHERE is_deleted = 0"
            )
            total_messages = cursor.fetchone()[0]

            # Challenge statistics
            cursor.execute(
                "SELECT COUNT(*) FROM study_challenges WHERE is_active = 1"
            )
...
def create_new_study_group(
    creator_id: int, name: str, description: str = "", language: str = "sw"
) -> Optional[int]:
    """Create a new study group"""
    return community_manager.create_study_group(
        creator_id, name, description, language
    )
...
def post_group_message(
    user_id: int, group_id: int, message: str
) -> Optional[int]:
